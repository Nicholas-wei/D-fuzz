...
Value* expr(const target_info target,Json::Value cond,Instruction &I){
  ...
  Json::Value lr_val = cond["lr"];
            ...
            if(lr_val[0]["l"].type() == Json::intValue || lr_val[0]["l"].type() ==Json::uintValue){
              ...
              Value* tmp_left = left;
                ...
                if(isa<AllocaInst>(tmp_left)){
                    ...
                    left = builder.CreateLoad(my_type,tmp_left);
            ...
            if(lr_val[0]["r"].type() == Json::intValue || lr_val[0]["r"].type() == Json::uintValue){
              ...
              Value* tmp_right = right;
                ...
                if(isa<AllocaInst>(tmp_right)){
                    ...
                    right = builder.CreateLoad(my_type, tmp_right);
    ...
    if(op == "&&"){
        ...
        Value *cmp_result = builder.CreateICmp(ICmpInst::ICMP_UGT,left,right);
        debug_print("1");
        Value *result = builder.CreateSelect(cmp_result,left,right);
        inst_blocks+=2;
        return result;
    }
    if(op == "||"){
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  std::map<std::string, int> bb_to_dis_tar1;
  std::map<std::string, int> bb_to_dis_tar2;
      ...
      while (getline(cf, line)) {
        ...
        if(!is_mul_target){
            ...
            if(line.find("@target2")!= std::string::npos){
              printf(GREEN "@target2\n" RESET);
              target_index = 2;
              continue;
            }
            // we only cares bb_to_dis
            if(target_index == 1){

                std::size_t pos = line.find(",");
                std::string bb_name = line.substr(0, pos);
                int bb_dis = (int) (10.0 * atof(line.substr(pos + 1, line.length()).c_str()));
                bb_to_dis_tar1.emplace(bb_name, bb_dis);
                //printf(CYAN "target1 store\n" RESET);
                basic_blocks.push_back(bb_name);
            }
                ...
                std::string bb_name = line.substr(0, pos);
                int bb_dis = (int) (10.0 * atof(line.substr(pos + 1, line.length()).c_str()));
                bb_to_dis_tar2.emplace(bb_name, bb_dis);
                //printf(BLUE "target2 store\n" RESET);
                basic_blocks.push_back(bb_name);
            }
            else{
                // printf(RED,"target_index is %d\n",RESET,target_index);
  ...
  if (is_aflgo_preprocessing) {
    ...
    ConstantInt *MapDist2Loc = ConstantInt::get(LargestType, MAP_SIZE+24);
    ...
    for (auto &F : M) {
      // this instrument the distance, we ran it twice for multiple distances
      ...
      for (auto &BB : F) {
        ...
        if (is_aflgo) {
          ...
          if (!bb_name.empty()) {
            ...
            if (find(basic_blocks.begin(), basic_blocks.end(), bb_name) == basic_blocks.end()) {
              ...
              // do not instrument temporary block, if name is the same, we don't care here
              if (is_selective)
                continue;
              ...
              // we don't set random here. Instrument them all
              if(!is_mul_target){
                std::map<std::string,int>::iterator it;
                for (it = bb_to_dis.begin(); it != bb_to_dis.end(); ++it)
                  if (it->first.compare(bb_name) == 0)
                ...
                // if not multiple target, do instrumentation twice, this is the first time
                  std::map<std::string,int>::iterator it;
                  for (it = bb_to_dis_tar1.begin(); it != bb_to_dis_tar1.end(); ++it)
                    if (it->first.compare(bb_name) == 0)
                      distance = it->second;
        ...
        if (distance >= 0) {
          ...
          // determin whether is mul_target
          if(!is_mul_target  || (is_mul_target && (target_index == 1))){
              ...
              IRB.CreateStore(dist_store, MapDistPtr)
                  ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
              printf(BLUE "dist1/single distance create store\n" RESET);
      ...
      // the second time BB iteration: instrument if multiple target and target==2. Only instrument distance here
      
      if(is_mul_target && is_aflgo){
          target_index = 2;
      

      // here is time for multiple target and the second time
      for(auto &BB : F) {
        int distance = -1;
          ...
          if (!bb_name.empty()) {
            ...
            if (find(basic_blocks.begin(), basic_blocks.end(), bb_name) == basic_blocks.end()) {
              ...
              // do not instrument self block, if name is the same, we don't care here
              if (is_selective)
                continue;
                ...
                // if not multiple target, do instrumentation twice, this is the first time
                  std::map<std::string,int>::iterator it;
                  for (it = bb_to_dis_tar2.begin(); it != bb_to_dis_tar2.end(); ++it)
                    if (it->first.compare(bb_name) == 0)
                      distance = it->second;
            }
          }
          ...
          // determin whether is mul_target
          assert(is_mul_target && target_index == 2);
          if(is_mul_target  && target_index == 2){
              Value *MapDistPtr = IRB.CreateBitCast(
              IRB.CreateGEP(MapPtr, MapDist2Loc), LargestType->getPointerTo());
              LoadInst *MapDist = IRB.CreateLoad(MapDistPtr);
              MapDist->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
              ...
              IRB.CreateStore(dist_store, MapDistPtr)
                  ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
              printf(CYAN "dist2 distance create store\n" RESET);
